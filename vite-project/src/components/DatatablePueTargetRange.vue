    ...
    <div v-if="tableData || isLoading">
        ...
        <div v-else class="table-responsive table-freeze bg-white">
            <table class="table table-bordered table-head-primary table-collapsable">
                <thead>
                    <tr>
                        <th :colspan="categoryList.length + 1" class="px-4">
                            <h5 class="f-w-700 text-start mb-0">Range PUE</h5>
                        </th>
                    </tr>
                    <tr>
                        <th class="bg-success sticky-column">Lingkup Kerja</th>
                        <th v-for="item in categoryList" class="bg-success">{{ item.title }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in tableData" :class="getRowClass(item)">
                        ...
                        <td v-for="catItem in item.categoryCount" class="middle text-center">{{ catItem }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
