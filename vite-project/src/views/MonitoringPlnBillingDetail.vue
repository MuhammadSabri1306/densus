...
import DialogPlnBillingForm from "@components/DialogPlnBillingForm.vue";
...
const fetch = () => {
    ...
    plnStore.getBillOnLocation(locationId.value, response => {
...
const openFormEdit = rowIndex => {
    currBilling.value = data.value[rowIndex];
    showForm.value = true;
};
...
<template>
    <div>
        ...
        <div class="container-fluid">
            <div v-if="userRole == 'admin'" class="d-flex justify-content-end mb-4">
                <button type="button" @click="openFormNew" class="btn btn-icon btn-outline-info">
                    ...
                    <span class="ms-1">Input Bill Baru</span>
                </button>
            </div>
        ...
        <DialogPlnBillingForm v-if="showForm" :billing="currBilling" @close="onFormClose" @save="fetch" />
    </div>
</template>