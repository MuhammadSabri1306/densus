...
$witelSummary = array_reduce($data, function($result, $row) {
    $witelCode = $row['location']['witel_kode'];
    if(!isset($result[$witelCode])) {
        $result[$witelCode] = [
            'performance' => [],
            'performance_summary' => [],
    ...
    for($i=0; $i<count($row['performance']); $i++) {
        if(!isset($result[$witelCode]['performance'][$i])) {
            $result[$witelCode]['performance'][$i] = [];
        }
        array_push($result[$witelCode]['performance'][$i], $row['performance'][$i]['percentage']);
    }

    array_push($result[$witelCode]['performance_summary'], $row['performance_summary']);
    array_push($result[$witelCode]['performance_summary_yearly'], $row['performance_summary_yearly']);

    array_push($result[$witelCode]['pue_online'], $row['pue']['online']);
    array_push($result[$witelCode]['pue_offline'], $row['pue']['offline']);
});

$witelData = [];
foreach($witelSummary as $witel) {
    $rowWitel = [];

    for($i=0; $i<count($witel['performance']); $i++) {
        if(count($witel['performance'][$i]) < 1) {
            $rowWitel['performance'][$i] = null;
        } else {
            $rowWitel['performance'][$i] = array_sum($witel['performance'][$i]) / count($witel['performance'][$i]);
        }
    }

    if(count($witel['performance_summary']) < 1) {
        $rowWitel['performance_summary'] = null;
    } else {
        $rowWitel['performance_summary'] = array_sum($rowWitel['performance_summary']) / count($rowWitel['performance_summary']);
    }
}
