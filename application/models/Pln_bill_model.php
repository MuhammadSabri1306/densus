<?php
class Pln_bill_model extends CI_Model
{
    private $tableName = 'pln_bill';
    ...
    private $fillable_fields = [
        ...
        'main' => [
            'lwbp_awal' => ['float', 'required'],
            'lwbp_akhir' => ['float', 'required'],
            'wbp_awal' => ['float', 'nullable'],
            'wbp_akhir' => ['float', 'nullable'],
            'kvarh_awal' => ['float', 'nullable'],
            'kvarh_akhir' => ['float', 'nullable'],
            'fkm' => ['int', 'required']
        ]
    ];
    ...
    private function get_filter($filter)
    {
        ...
        if(is_array($filter) && $filter['location']) $appliedFilter['bill.id_location'] = $filter['location'];
        if(is_array($filter) && $filter['id']) $appliedFilter['bill.id'] = $filter['id'];
    ...
    public function get($filter = null)
    {
        ...
        $this->db
            ->select('bill.*, '.implode(', ', $locFields))
            ->from("$this->tableName AS bill")
            ->join("$this->tableLocationName AS loc", 'loc.id=bill.id_location')
    ...
    public function get_avg($filter = null)
    {
        ...
        $locFields = ['loc.id_pel_pln', 'loc.nama_pel_pln', 'loc.tarif_pel_pln', 'loc.daya_pel_pln',
            'loc.lokasi_pel_pln', 'loc.alamat_pel_pln', 'loc.gedung', 'loc.divre_kode', 'loc.divre_name',
            'loc.witel_kode', 'loc.witel_name', 'loc.sto_kode', 'loc.sto_name', 'loc.tipe', 'loc.rtu_kode'];
        $selectQuery = 'SELECT DISTINCT bill.id_location, '.implode(', ', $locFields).', '.implode(', ', $fields);

        $qArray = [
            $selectQuery,
            "FROM $this->tableLocationName AS loc",
            "LEFT JOIN $this->tableName AS bill ON bill.id_location=loc.id"
        ];
